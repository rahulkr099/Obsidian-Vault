...
<script>
    ...
    function calculate() {
        ...
        //Get full expression value (example: "12+3*5")
            ...
            let stack = []; // Stack to store processed values
            let i = 0; // Pointer to iterate tokens
            ...
            while (i < tokens.length) {
                ...
                //Get previous number from stack
                    let prev = stack.pop();
                    
                //Get next number from tokens
				...
				//Perform multiplication or division
                    let result = token === "*"
                        ? prev * next //If *, multiply
                        : prev / next; //If /, divide
                        
				//Push result back into stack
                    stack.push(result);
                    //Move pointer ahead by 2 (skip operator and next number)
                    i += 2;
                } else {
                //If token is + or -, keep it as operator
                //Else convert number string -> float
            ...
            for (let j = 1; j < stack.length; j += 2) {
                const operator = stack[j];  //Operator (+ or -)
                const number = stack[j + 1]; //Next number
